# RBI Web Isolation (Neko 기반 원격 브라우저 격리)

첨부된 **Spector(RBI) 제품 소개서**의 핵심 개념을 오픈소스 [**Neko (m1k1o/neko)**](https://github.com/m1k1o/neko)
위에 구현한 **원격 브라우저 격리(Remote Browser Isolation)** MVP입니다.
...
│  사용자 PC   │ ───────────────▶ │   Gateway (Node) │ ───────────────────────▶ │  Neko Browser     │
...
|---|---|
| 격리 브라우저 (컨테이너, 흔적 폐기) | **Neko** 컨테이너 (`neko-browser/`) |
| 원격 화면 스트리밍 (WebRTC) | Neko WebRTC (iframe 임베드) |
...
| 클립보드 통제 (PC↔RB) | `gateway/src/policy.js` clipboard 정책 + CDP |
...
```
(격리 브라우저 화면은 Neko 컨테이너가 필요하므로 Compose 사용을 권장합니다.)

## 디렉터리

```
rbi-web-isolation/
├─ docker-compose.yml          # 전체 스택 (neko + gateway)
├─ .env.example
├─ gateway/                    # Node.js 백엔드: 인증·정책·감사·프록시·관리 API
├─ frontend/                   # React 워크스페이스 (인포바/워터마크) + 관리자 콘솔
├─ neko-browser/               # Neko 확장 이미지 (원격 디버깅 + 관리정책 마운트)
├─ k8s/                        # 쿠버네티스 확장 매니페스트
